...
public class AccionSeleccionar {
	...
	public static void seleccionarComics(boolean esPrincipal) {
		...
		getReferenciaVentana().getImagenComic().setOnMouseClicked(event -> {
			ComicGradeo comic = newSelection;
	...
	public static void verBasedeDatos(boolean completo, boolean esAccion, ComicGradeo comic) {
		...
		if (ComicManagerDAO.countRows() > 0) {
			if (completo) {
				...
				List<ComicGradeo> listaComics = ComicManagerDAO.verLibreria(sentenciaSQL);

				FuncionesTableView.tablaBBDD(listaComics);

			} else {

				List<ComicGradeo> listaParametro = listaPorParametro(comic, esAccion);
	 ...
	 * @throws SQLException
	 */
	public static List<ComicGradeo> listaPorParametro(ComicGradeo datos, boolean esAccion) {
		...
		List<ComicGradeo> listComic = FXCollections
